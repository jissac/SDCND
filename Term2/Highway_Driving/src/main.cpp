...
double rel_vel = 0.0;
// initialize road and vehicle parameters
const double speed_limit = 49.5; //mph
double front_buffer = 30.0; // meters
double rear_buffer = -15.0; // meters
...
////////////////////////          
          ...
          // keep track of other cars in host car's vicinity
          bool current_lane_car = false;
          bool left_lane_car = false;
          bool right_lane_car = false;
          
          // loop through all objects in the sensor_fusion vector and find each vehicle's
          // location and reference velocity to compare to the host vehicle
          for(int i = 0; i <sensor_fusion.size(); i++)
          {
            // determine which lane the other vehicle is in
            float d = sensor_fusion[i][6];
            int object_lane = -1;
            if(d >= 0.0 && d < 5.0)
            {
              object_lane = 0; //left lane
            } else if (d >=5.0 && d < 8.0) {
              object_lane = 1; // center lane
            } else {
              object_lane = 2; // right lane
            }
            
            // determine if another car is in the current lane
            ...
            if(object_lane == lane && !current_lane_car)
            {
              current_lane_car = s_diff > 0 && s_diff < front_buffer;
            }else if(object_lane == lane - 1 && !left_lane_car) {
              left_lane_car = s_diff < front_buffer && s_diff > rear_buffer;
            }else if(object_lane == lane - 2 && !right_lane_car) {
              right_lane_car = s_diff < front_buffer && s_diff > rear_buffer;
            }
...
//             } else if(object_lane == lane-1 && !left_lane_car) {
//               left_lane_car = true;
//             } else if(object_lane == lane+1 && !right_lane_car) {
//               right_lane_car = true;
//             }
...
//             }
          ...
          if (current_lane_car)
          {
            if(lane > 0 && !left_lane_car)
            {
              --lane; // lane change left
            }else if(lane < 2 && !right_lane_car){
              ++lane; // lane change right
            }else{
              rel_vel -= 0.624; // reduce speed
            }
          }
          else {
            if(rel_vel < speed_limit)
            {
              rel_vel += 0.304;// speed up
            }  
          }
          ...
          // what is previous path size, if empty use the car as starting reference
          if(prev_size < 2)
          {
...
//////////////////////// 
          ...
          {
            double N = (target_dist/(.02*rel_vel/2.24));
